...
CREATE TABLE IF NOT EXISTS profileragent.profilers (
  ...
  metamanagementschema VARCHAR(6000) NOT NULL,
  dryrunschema VARCHAR(6000) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS profileragent.profilerinstances (
  ...
  FOREIGN KEY(profilerid) REFERENCES profileragent.profilers (id)
);

CREATE TABLE IF NOT EXISTS profileragent.asset_selector_definitions (
  id  BIGSERIAL NOT NULL PRIMARY KEY,
  name VARCHAR(255) NOT NULL UNIQUE,
  profiler_instance_name VARCHAR(255) NOT NULL UNIQUE,
  ...
  source_lastsubmitted BIGINT,
  FOREIGN KEY(profiler_instance_name) REFERENCES profileragent.profilerinstances (name)
);
...
CREATE TABLE IF NOT EXISTS profileragent.jobs (
  ...
  FOREIGN KEY(profilerinstanceid) REFERENCES profileragent.profilerinstances (id)
);
...
CREATE TABLE IF NOT EXISTS profileragent.assetjobshistory (
  ...
  status VARCHAR(255) NOT NULL ,
  lastupdated TIMESTAMP DEFAULT now() NOT NULL,
  FOREIGN KEY(jobid) REFERENCES profileragent.jobs (id)
);
...
CREATE TABLE IF NOT EXISTS profileragent.change_notification_log (
  id BIGSERIAL NOT NULL PRIMARY KEY,
