...
import livy.session.models.LivyRequests.{CreateSession, SubmitStatement}
    ...
    withAuth(ws.url(sessionUrl))
      ...
      response => {
        response.status match {
          ...
          case _ =>
            Future.failed(new Exception("Failed to get session state from livy"))
        }
      }
      )
  }
    ...
    withAuth(ws.url(sessionUrl))
      ...
      response => {
        response.status match {
          ...
          case _ =>
            Future.failed(new Exception("Failed to get session from livy"))
        }
      }
      )
  }
        ...
        response.status match {
          ...
          case status if (status == Status.NOT_FOUND) || (status == Status.INTERNAL_SERVER_ERROR
            ...
            Future.failed(new LivySessionNotRunningException("Session not found or dead." +
              " Initializing session again"))
          case _ =>
            Future.failed(new Exception("Failed to get Statement from livy"))
        }
      }
      )
  }
