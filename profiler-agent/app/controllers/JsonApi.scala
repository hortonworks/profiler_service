...
import domain.{Errors, InstanceNotFoundException, ProfilerConfigValidationException}
...
trait JsonApi {
  ...
  def mapError: PartialFunction[Throwable, Result] = {
    ...
    case e: ProfilerConfigValidationException =>
      BadRequest(Json.toJson(Errors(e.code, e.message)))
    case e:InstanceNotFoundException =>
      NotFound(Json.toJson(Errors(e.code, e.message)))
    case (th: Throwable) =>
