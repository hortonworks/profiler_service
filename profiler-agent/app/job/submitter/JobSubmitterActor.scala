  ...
  private def addAndProcessEnvelop(envelops: Seq[AssetEnvelop]) = {
    ...
    def processEnvelops = (envelop: AssetEnvelop) => {
      if (queue.size <= maxQueueSize) {
        ...
        if (isUnique && queue.size >= maxQueueSize) {
    ...
    envelops.foreach(processEnvelops)

    if (acceptedEnvelops.nonEmpty && rejectedEnvelops.nonEmpty) {
      sender ! PartialRejected(rejectedEnvelops, s"Queue size is full or the envelop was duplicate with a lesser priority. Current Q size: ${queue.size}")
