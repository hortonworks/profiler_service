...
class LivyJobRunner(extraJars: List[String], livyConf: Map[String, String], ws: WSClient, jobDataBuilder: JobDataBuilder, secured: Boolean) extends JobRunner with WSResponseMapper {
  ...
  def addExtraJars(profilerConf: JsObject): JsObject = {
    val currentDependencyJars: List[String] = (profilerConf \ "jars").asOpt[List[String]].getOrElse(Nil)
    val updatedJars = Json.obj("jars" -> (currentDependencyJars ::: extraJars))
    profilerConf ++ updatedJars
  }
  ...
  override def start(profiler: ProfilerAndProfilerInstance, jobTask: JobTask): Future[ProfilerJob] = {
    ...
    val confgiWithExtraJars: JsObject = addExtraJars(config)
    val data = jobDataBuilder.getJson(profiler, jobTask)
    val jsObject = Json.obj("queue" -> profiler.profilerInstance.queue) ++
      confgiWithExtraJars ++ Json.obj("args" -> Json.toJson(Seq(data.toString())))
