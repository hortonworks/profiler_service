...
class LivyInteractiveRunner @Inject()(configuration: Configuration, ws: WSClient, actorSystem: ActorSystem, interactor: LivyInteractor) extends WSResponseMapper {
  ...
  def sessionStatus(sessionType: String): Future[Int] = {
    val sessionProvider =
      sessionType.toLowerCase() match {
        case "write" => writeSessionProvider
        case _ => readSessionProvider
      }

    val sessionId = ask(sessionProvider, GetSessionId)(Timeout(5 seconds))
    sessionId.flatMap {
      case SessionNotAvailable => Future.failed(
        new LivySessionNotAvailableException()
      )
      case InvalidStateRestartRequired => Future.failed(
        new LivySessionInconsistentStateException()
      )
      case SessionId(id) =>
      ...
      case SessionId(id) =>
        ...
        eventualResult onFailure {
          case error: LivySessionNotRunningException =>
            Logger.error(s"Session $id dead. Trying to restart,", error)
            sessionProvider ! SessionYouGaveIsDead(id)
          case otherError =>
            Logger.error(s"Query to livy session $id failed", otherError)
            sessionProvider ! ErrorEncounteredIndication(id)
        }
        eventualResult
    }
  }
    ...
    interactor.getStatement(sessionId, statementId).flatMap(
      statement => {
        if (statement.state == StateOfStatement.available) {
          if (statement.output.isDefined) {
            ...
            output match {
              ...
              case error: ErrorStatementOutput =>
                Future.failed(new Exception(s"Execution of statement $statementId to session $sessionId failed," +
                  s" cause : ${error.evalue} , stacktrace:${error.traceback.mkString(";")}"))
            }
          }
          else {
          ...
          Future.failed(new Exception(s"Statement $statementId to session $sessionId is either cancelled or" +
  ...
  def refreshAllTables(): Future[JsValue] = {
    val statement =
      """
        |spark.sql("SHOW TABLES").filter("isTemporary=True").select("tableName").collect.map(_.getString(0)).foreach {
        |  table =>
        |     {
