  ...
  private def onSubmitResponse(submitResult: JobSubmitterMessage): Future[Int] = {
    submitResult match {
      ...
      case partialRejected: PartialRejected =>
        akka.pattern.after(60 seconds, using = system.scheduler)(submitAssets(partialRejected.assetEnvelops.map(_.asset)).flatMap(onSubmitResponse))
      case rejected: AssetRejected =>
        akka.pattern.after(60 seconds, using = system.scheduler)(submitAssets(rejected.assetEnvelops.map(_.asset)).flatMap(onSubmitResponse))
    }
  }
