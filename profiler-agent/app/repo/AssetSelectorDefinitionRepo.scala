...
@Singleton
class AssetSelectorDefinitionRepo @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) {
  ...
  final class AssetSelectorDefinitionsTable(tag: Tag) extends Table[AssetSelectorDefinition](tag, Some("profileragent"), "asset_selector_definitions") {
    ...
    override def * = (id, name, profilerInstanceName, selectorType, config, sourceDefinition, pickFilters, dropFilters, lastUpdated) <> ((AssetSelectorDefinition.apply _) tupled, AssetSelectorDefinition.unapply)
  }
