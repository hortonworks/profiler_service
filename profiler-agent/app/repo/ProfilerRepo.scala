...
import domain.AssetType.AssetType
...
@Singleton
class ProfilerRepo @Inject()(protected val dbConfigProvider: ProfilerDatabaseConfigProvider) {
  ...
  implicit val metaManagementSchemaToJsonString = MappedColumnType.base[Option[ProfilerMetaConfiguration], String](
    {
      case Some(x) => Json.toJson(x).toString()
      case None => JsNull.toString()
    },
    d => {
      Json.parse(d) match {
        case JsNull => None
        case x: JsValue => x.validate[ProfilerMetaConfiguration].asOpt
      }
    }
  )


  implicit val dryRunSchemaToJsonString = MappedColumnType.base[Option[DryRunSchemaForProfiler], String](
    {
      case Some(x) => Json.toJson(x).toString()
      case None => JsNull.toString()
    },
    d => {
      Json.parse(d) match {
        case JsNull => None
        case x: JsValue => x.validate[DryRunSchemaForProfiler].asOpt
      }
    }
  )
  ...
  final class ProfilersTable(tag: Tag) extends Table[Profiler](tag, Some("profileragent"), "profilers") {
    ...
    def metaManagementSchema = column[Option[ProfilerMetaConfiguration]]("metamanagementschema")

    def dryRunSchema = column[Option[DryRunSchemaForProfiler]]("dryrunschema")

    def * = (id, name, version, jobType, assetType, profilerConf, user, description, created, metaManagementSchema, dryRunSchema) <> (Profiler.apply _ tupled, Profiler.unapply)
  }
