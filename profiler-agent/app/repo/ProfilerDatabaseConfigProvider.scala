...
@Singleton
class ProfilerDatabaseConfigProvider @Inject()(dbConfigProvider: DatabaseConfigProvider, config: Configuration) {
  ...
  val dbConfig = if (passwordEncrytionEnabled) {
    createDecryptedDbConfig
  } else dbConfigProvider.get[JdbcProfile]

  private def decrypt(value: String) = {
    val credentailProvider = new ProfilerCredentialProvider(credentialFile)
    credentailProvider.resolveAlias(value)
  }

  private def createDecryptedDbConfig: DatabaseConfig[JdbcProfile] = {
