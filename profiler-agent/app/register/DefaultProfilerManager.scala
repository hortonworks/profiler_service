...
class DefaultProfilerManager @Inject()(profilerRepo: ProfilerRepo,
  ...
  private val isAutoRegistrationEnable = config.getBoolean("profiler.autoregister").getOrElse(false)
  ...
  private def profilerInstanceIfNotExists(profilerInstance: ProfilerInstance): Future[Option[ProfilerInstance]] = {
    profilerInstanceRepo.findByNameAndVersion(profilerInstance.name, profilerInstance.version).map {
      p =>
        Logger.info(s"Profiler Instance with ${profilerInstance.name} : ${profilerInstance.version} exists : ${p.isDefined}")
        p match {
          case Some(pr) => None
  ...
  private def saveProfilerIfNeeded(profilerDir: File, profiler: Profiler): Future[Profiler] = {
    profilerRepo.findByNameAndVersion(profiler.name, profiler.version).flatMap {
      ...
      case None => {
        uploadProfiler(profilerDir, profiler.name, profiler.version)
  ...
  private def registerProfiler(profilerDir: File): Future[Seq[ProfilerInstance]] = {
    ...
    val tempProfiler = getProfiler(profilerDir)
    val profilerHdfsDir = getProfilerHdfsDir(tempProfiler.name, tempProfiler.version)
    val profiler = saveProfilerIfNeeded(profilerDir, getProfilerAfterSubstitution(profilerDir, profilerHdfsDir))
    profiler.flatMap(p => saveProfilerInstances(profilerDir, p))
      .map {
        pi =>
          saveSelectors(profilerDir)
          pi
      }
  }
