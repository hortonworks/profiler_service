...
class SpnegoAuthenticationFilter @Inject()(implicit val mat: Materializer, ec: ExecutionContext, config: Configuration) extends Filter {

  private val spnegoAuthenticatorOption = getSpnegoAuthenticator()

  def contructCookie(token: Token) = {
    val signatureSecret = config.getString("dpprofiler.spnego.signature.secret").getOrElse("dpprofilersecret")
    ...
    val tokens = new Tokens(tokenValidity, signatureSecret.getBytes(UTF_8))
  ...
  def getSpnegoAuthenticator(): Option[SpnegoAuthenticator] = {
    val isKerberosEnabled = config.getBoolean("dpprofiler.secured").getOrElse(false)
    ...
    if (isKerberosEnabled) {
      ...
      val configPrincipalValue = configPrincipal.getOrElse("")
      ...
      val keytab = config.getString("dpprofiler.spnego.kerberos.keytab").getOrElse("")
      ...
      val tokens = new Tokens(tokenValidity, signatureSecret.getBytes(UTF_8))
      Some(new SpnegoAuthenticator(principal, keytab, debug, domain, path, tokens, cookieName))
    } else {
      Logger.info("Kerberos is not enabled for profiler agent. Skipping SPNEGO authentication filter")
      None
    }
  }
  ...
  def apply(nextFilter: RequestHeader => Future[Result])
    ...
    if(spnegoAuthenticatorOption.isDefined){
      Logger.debug("Invoking spnego authentication filter")
      performSpnegoAuthentication(nextFilter, requestHeader, spnegoAuthenticatorOption.get)
    }
    else{
