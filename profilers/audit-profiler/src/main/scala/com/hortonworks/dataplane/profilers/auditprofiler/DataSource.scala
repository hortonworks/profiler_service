...
object DataSource {
  ...
  def getDF(sparkSession: SparkSession, job: AuditQueryJob, properties: Map[String, String]): DataFrame = {
    job.source match {
  ...
  private def getAssetRollupDF(sparkSession: SparkSession, job: AuditQueryJob, properties: Map[String, String]) = {
    ...
    sparkSession.sqlContext.read.parquet(dirs: _*)
  }
