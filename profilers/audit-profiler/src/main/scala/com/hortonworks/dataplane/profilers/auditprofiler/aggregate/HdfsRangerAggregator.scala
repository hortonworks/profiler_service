...
case class HdfsAggregate(resource: String, count: Int, data: AggData, user: Map[String, AggData])

object HdfsRangerAggregator {
  ...
  def aggregateDF(df: DataFrame)(implicit session: SparkSession): DataFrame = {
    ...
    val dfRdd = df.rdd.filter(_.getAs[String]("resource") != null).groupBy {
      f =>
        f.getAs[String]("resource")
    }.mapValues {
      rows =>
      ...
      f =>
        HdfsAggregate(f._1, f._2._3, f._2._1, f._2._2)
    }
