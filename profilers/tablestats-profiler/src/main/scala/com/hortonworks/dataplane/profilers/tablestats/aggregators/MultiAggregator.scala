...
import com.hortonworks.dataplane.profilers.tablestats.utils.{ColumnMetrics, Metrics, MetricsSeq}
...
case class MultiAggregator[C, IN](aggregators: Seq[Aggregator[C, IN]])
  ...
  override def result(acc: Seq[C]): MetricsSeq = new MetricsSeq((acc, aggregators).zipped.map {
    (c, agg) =>
      agg.result(c).asInstanceOf[ColumnMetrics]
  })

  override def getState(accs: Seq[C]): StateSeq = StateSeq(aggregators.zip(accs).map {
    case(agg, acc) => agg.getState(acc).asInstanceOf[StateWithColumnNameInfo]
  })
}
