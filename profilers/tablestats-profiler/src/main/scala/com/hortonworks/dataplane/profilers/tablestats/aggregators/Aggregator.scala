...
trait Aggregator[C, IN] {
  ...
  def stateExecutor(rdd: RDD[IN])(implicit clazz: ClassTag[C]): AggregateState = {
    getState(rdd.treeAggregate(initialElement)(mergeValue, mergeCombiners))
  }
  ...
  def getState(acc: C): AggregateState
...
object Aggregator {
  ...
  private[Aggregator] case class RowInputAggregator[C, IN](aggregator: Aggregator[C, IN],
    ...
    override def getState(acc: C): StateWithColumnNameInfo = new StateWithColumnNameInfo(colName, aggregator.getState(acc))
