...
import com.hortonworks.dataplane.profilers.tablestats.aggregators.{Aggregator, QuantileState}
...
class QuartileAggregator(probabilities: Array[Double], relativeError: Double)
  ...
  override def getState(acc: QuantileSummaries) = QuantileState(acc.compressThreshold,
    acc.relativeError, acc.sampled, acc.count, acc.getHeadSampled())
}
