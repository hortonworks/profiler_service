...
object AggregatorHelper {
  ...
  def getAggregators(df: DataFrame) = {
    val aggregators = aggregatorProviders.map(_.get(df, Nil)).flatten
    new MultiAggregator(aggregators.map(_.asInstanceOf[Aggregator[Any, Row]]))
  }

  def getPartitionedAggregators(df: DataFrame) = {
    val aggregators = partitionedAggregatorProvider.map(_.get(df, Nil)).flatten
    new MultiAggregator((aggregators).map(_.asInstanceOf[Aggregator[Any, Row]]))
  }

  def getFrequentCountAndHistogramAggregators(df: DataFrame, metrics: Seq[Metrics]) = {
    new MultiAggregator(
      (FrequentItemsCountAggregatorProvider.get(df, metrics) ++
        HistogramAggregatorProvider.get(df, metrics)).map(_.asInstanceOf[Aggregator[Any, Row]])
    )
  }
