...
object InputParser {
  ...
  def get(input: String): InputArguments = {
    ...
    val metadataDumpLocation = (clusterConfigs \ "assetMetricsPath").as[String] + pathSuffix

    val kerberosCredentials = ParseKerberosCredentials.getkerberosCredentials(json)

    InputArguments(metastoreUrl, clusterName, metadataDumpLocation, kerberosCredentials)
  }

}

case class InputArguments(metaStoreUrl:String, clusterName:String, metadataDumpLocation:String,
