...
object HiveTableMetadata extends LazyLogging {
  ...
  def getNumPartitions(session: SparkSession, tableName: String, dbName: String): Long = {
    var partitionCount = 0L
    try {
      partitionCount = session.sql("show partitions " + dbName + "." + tableName).count()
    } catch {
      case e: Exception =>
    }
    partitionCount
  ...
  def getRDDWithMetadata(session: SparkSession, tableName: String, dbName: String): RDD[Row] = {
    logger.info("Constructing RDD row for table {}.{}", dbName, tableName)
    val filteredMetadataMap = session.sql("describe formatted " + dbName + "." + tableName).
      filter(x => attributeFilter(x(0).toString)).collect().map(x => (x(0).toString -> x(1).toString)).toMap
    val numPartitions = getNumPartitions(session, tableName, dbName)
  ...
  def collectMetadata(session: SparkSession, filePath: String): Unit = {
    ...
    val fullMetadata = databaseList.flatMap(row => {
      ...
      val tables = session.sql("show tables")
      val tableMetadataData = tables.collect().map(r => {
        val tableName = r(1).toString()
        logger.info("Collecting metadata for table {}.{}", database, tableName)
        try {
          Some(getRDDWithMetadata(session, tableName, database))
        } catch {
          case e: Exception =>
    ...
    val mergedRDD = session.sparkContext.union(fullMetadata)
    logger.info("Converting RDD to Dataframe and reducing the number of partitions")
    val mergedDF = session.sqlContext.createDataFrame(mergedRDD, schemaRDD).coalesce(5)
    logger.info("Attempting to save the dataframe to HDFS path {}", filePath)
    writeResultsToHDFS(filePath, mergedDF)
  }

  private def writeResultsToHDFS(filePath: String, mergedDF: Dataset[Row]) = {
    mergedDF.cache()
    val today: String = {
  ...
  def readMetadataAndExecuteQueries(session: SparkSession, filePath: String): Unit = {
    val optionalSeperator = addSeperatorToPath(filePath)
    val snapshotPath = filePath + s"${optionalSeperator}snapshot"
    val df = session.sqlContext.read.parquet(snapshotPath)
    MetadataQuery.values.map(queryType => {
      logger.info("Results for QueryType => {}", queryType.toString)
      val t0 = System.currentTimeMillis()
      CannedCollectionQueries.getQueryResults(session, df, queryType).map(println)
      val t1 = System.currentTimeMillis()
      logger.info("Time taken => {}", ((t1 - t0) / 1000.0).toString)
    })
  }
}