...
/**
  * Created by visharma on 5/30/17.
  */
object AtlasPersister extends LazyLogging{
  def persistToAtlas(atlasURL: String, atlasRestRequestParams: List[(String, String)],clusterName: String, dbName: String, tableName: String,
                     collectedMetrics: Map[String, Utils.BaseMetrics with Product with Serializable],
    ...
    val column_entities = collectedMetrics map {
      entry => {
        ...
        val sparkDataType = columnMetrics.columnType
        val hiveColumnType = try {
          DataType.sparkToHiveDataTypeMap(sparkDataType)
        } catch{
          case e:Exception => sparkDataType
        }

        hiveColumnReferenceable.set(Constants.TypeAttribute, hiveColumnType)
        ...
        columnProfileData.set("nonNullCount", columnMetrics.nonNullCount)
        columnProfileData.set("cardinality", columnMetrics.cardinality)

        DataType.getDataType(sparkDataType) match {
          case DataType.Numeric => {
            val numericMetrics = columnMetrics.asInstanceOf[Utils.NumericMetrics]
            columnProfileData.set("minValue", numericMetrics.minValue)
            columnProfileData.set("maxValue", numericMetrics.maxValue)
            columnProfileData.set("meanValue", numericMetrics.meanValue)
            columnProfileData.set("stdDeviation", numericMetrics.stdDeviation)
            columnProfileData.set("histogram", numericMetrics.histogram)
            columnProfileData.set("frequentItems", numericMetrics.frequentItems)
            columnProfileData.set("quartiles", numericMetrics.quartiles)
          }
          case DataType.String => {
            val stringMetrics = columnMetrics.asInstanceOf[Utils.StringMetrics]
            columnProfileData.set("lengthMinValue", stringMetrics.lengthMinValue)
            columnProfileData.set("lengthMaxValue", stringMetrics.lengthMaxValue)
            columnProfileData.set("lengthMeanValue", stringMetrics.lengthMeanValue)
            columnProfileData.set("lengthStdDeviation", stringMetrics.lengthStdDeviation)
            columnProfileData.set("histogram", stringMetrics.histogram)
            columnProfileData.set("frequentItems", stringMetrics.frequentItems)
          }

          case DataType.Boolean => {
            val booleanMetrics = columnMetrics.asInstanceOf[Utils.BooleanMetrics]
            columnProfileData.set("numTrue", booleanMetrics.numTrue)
            columnProfileData.set("numFalse", booleanMetrics.numFalse)
          }
        }
